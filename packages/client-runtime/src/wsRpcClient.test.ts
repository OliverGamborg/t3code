...
describe("createWsRpcClient", () => {
  ...
  it("delegates agent plan coordination requests to orchestration RPC methods", async () => {
    const planId = AgentPlanId.make("plan-coordination");
    const taskId = AgentTaskId.make("task-coordination");
    const rpcMethods = {
      [ORCHESTRATION_WS_METHODS.importAgentPlanOwnerOutput]: vi.fn(async () => ({
        planId,
        taskCount: 2,
        contractCount: 1,
        sequence: 1,
      })),
      [ORCHESTRATION_WS_METHODS.approveAgentPlanTasks]: vi.fn(async () => ({
        planId,
        launchedTaskIds: [taskId],
        skippedTaskIds: [],
        sequence: 2,
      })),
      [ORCHESTRATION_WS_METHODS.launchAgentPlanReadyWorkers]: vi.fn(async () => ({
        planId,
        launchedTaskIds: [taskId],
        skippedTaskIds: [],
        sequence: 3,
    ...
    await client.orchestration.importAgentPlanOwnerOutput({
      planId,
      source: "latest_owner_message",
    });
    await client.orchestration.approveAgentPlanTasks({ planId, taskIds: [taskId] });
    await client.orchestration.launchAgentPlanReadyWorkers({ planId, taskIds: [taskId] });
    await client.orchestration.sendAgentPlanWorkerMessage({
      planId,
      taskId,
    ...
    expect(rpcMethods[ORCHESTRATION_WS_METHODS.approveAgentPlanTasks]).toHaveBeenCalledWith({
      planId,
      taskIds: [taskId],
    });
    expect(rpcMethods[ORCHESTRATION_WS_METHODS.launchAgentPlanReadyWorkers]).toHaveBeenCalledWith({
      planId,
      taskIds: [taskId],
    });
    expect(rpcMethods[ORCHESTRATION_WS_METHODS.sendAgentPlanWorkerMessage]).toHaveBeenCalledWith({
      planId,
      taskId,
      kind: "progress_request",
      title: "Progress",
